...
public:
    MyAesCBC();
    MyAesCBC(int keySize, unsigned char* keyBytes); // 构造函数, 初始化密钥
    ~MyAesCBC();

    unsigned char State[4][4];

    DWORD OnAesEncrypt(QByteArray InBuffer, DWORD InLength, QByteArray &OutBuffer); // AES加密数据
    DWORD OnAesUncrypt(QByteArray InBuffer, DWORD InLength, QByteArray &OutBuffer); // AES解密数据
private:
    ...
    void SubBytes();             // S盒字节替换
    void InvSubBytes();          // 逆S合字节替换
    void ShiftRows();            // 行移位
    void InvShiftRows();
    void MixColumns();           // 列混淆
    void InvMixColumns();
    ...
    void KeyExpansion();         // ????
    unsigned char* SubWord(unsigned char* word); // 密钥S盒字节替换
